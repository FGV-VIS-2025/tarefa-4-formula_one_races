<script>
  ...
  $: if (f1data[mode + "Standings"]) {
    seasons = [
      ...new Set(f1data[mode + "Standings"].map((d) => d.season)),
    ].sort((a, b) => b - a);
  ...
  function buildSeries() {
    ...
    const standings = f1data[mode + "Standings"].filter(
      (d) => d.season === season,
    );
    ...
    series = d3.range(2, maxRound + 1).map((r) => {
      ...
      const topClimber = d3.least(deltas, (d) => d.delta) || {
        id: "",
        delta: 0,
      };
      const topDropper = d3.greatest(deltas, (d) => d.delta) || {
        id: "",
        delta: 0,
      };
      ...
      return {
        ...
        swapPct: swaps / n,
        topClimber: {
          name: nameMap.get(topClimber.id),
          delta: topClimber.delta,
        },
        topDropper: {
          name: nameMap.get(topDropper.id),
          delta: topDropper.delta,
        },
      };
    });
  ...
  function draw() {
    ...
    const g = svg
      .append("g")
      .attr("transform", `translate(${cfg.m.l},${cfg.m.t})`);

    const x = d3
      .scaleLinear()
      .domain(d3.extent(series, (d) => d.round))
      .range([0, innerW]);
    const y = d3
      .scaleLinear()
      .domain([0, d3.max(series, (d) => d.rvi)])
      .nice()
      .range([innerH, 0]);
    ...
    svg
      ...
      .attr("id", "glow")
      .html(
        '<feGaussianBlur stdDeviation="3" result="colored"/><feMerge><feMergeNode in="colored"/><feMergeNode in="SourceGraphic"/></feMerge>',
      );
    ...
    svg
      ...
      .style("fill", "#fff")
      .text(
        `RVI – Volatilidade de ${mode === "driver" ? "Pilotos" : "Construtores"} · ${season}`,
      );
  }
